...
import { ChevronDown, Download, FileCode, FileImage, FileText, FileType, LoaderCircle, Workflow } from 'lucide-react'
import { jsPDF } from 'jspdf'
import { exportDiagramFile } from '../services/api'
...
const OPTIONS = [
  ...
  // Interoperability formats: editable in other architecture tooling rather
  // than a flat picture. Both render in Confluence via their plugins.
  { type: 'drawio', label: 'draw.io / Confluence', icon: Workflow },
  { type: 'puml', label: 'PlantUML source', icon: FileCode },
]

/** Formats generated server-side from the frame plus the on-screen geometry. */
const INTEROP_FORMATS = new Set(['drawio', 'puml'])
 ...
 *
 * The draw.io and PlantUML formats are the exception: those are generated by
 * the backend from the active frame, with the live node geometry posted along
 * so the exported diagram keeps the arrangement that's on screen.
 *
 * @param {object} props
 * @param {() => object | null} props.getCy - Returns the live Cytoscape instance.
 * @param {string} [props.frame] - Active frame slug, sent with interop exports.
 * @param {(error: unknown) => void} [props.onError] - Called on export failure.
 */
function ExportButton({ getCy, frame = 'application', onError }) {
  ...
  const handleExport = async (type) => {
    ...
    setBusy(true)
    try {
      if (INTEROP_FORMATS.has(type)) {
        const placements = {}
        // ':visible' matters — filters hide elements via a display style rather
        // than removing them, so plain cy.nodes() would export filtered-out nodes.
        ...
        const { blob, filename } = await exportDiagramFile(type, frame, placements)
